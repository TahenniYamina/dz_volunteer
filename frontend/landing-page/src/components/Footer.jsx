import React from 'react'
import styles from './Footer.module.css'
import logo from '../assets/logo.png'

function Footer() {
  return (
    <footer className={styles.footer}>
      <div className={styles.container}>
        <div className={styles.topSection}>
          <div className={styles.brandSection}>
            <div className={styles.logo}>
              <img className={styles.logoImage} src={logo} alt="" />
            </div>
            <p className={styles.description}>
              Nous connectons ceux qui veulent aider avec ceux qui en ont besoin. 
              Ensemble, construisons un monde plus solidaire.
            </p>
<div className={styles.parent}>

  <div className={`${styles.child} ${styles.child1}`}>
    <button className={`${styles.button} ${styles.btn1}`}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 512 512"
        height="1em"
        fill="#000"
      >
        <path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z" />
      </svg>
    </button>
  </div>

<div className={`${styles.child} ${styles.child2}`}>
                <button className={`${styles.button} ${styles.btn2}`}>
                  
<svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512" fill="#ff00ff"><path d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z"></path></svg>
  </button>
</div>


  <div className={`${styles.child} ${styles.child3}`}>
    <button className={`${styles.button} ${styles.btn3}`}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        height="1em"
        viewBox="0 0 576 512"
        fill="#FF0000"
      >
        <path d="M549.7 124.1c-6.3-23.7-24.8-42.2-48.5-48.5C456.8 64 288 64 288 64S119.2 64 74.8 75.6c-23.7 6.3-42.2 24.8-48.5 48.5C14.7 168.6 14.7 256 14.7 256s0 87.4 11.6 131.9c6.3 23.7 24.8 42.2 48.5 48.5C119.2 448 288 448 288 448s168.8 0 213.2-11.6c23.7-6.3 42.2-24.8 48.5-48.5C561.3 343.4 561.3 256 561.3 256s0-87.4-11.6-131.9zM232 336V176l142 80-142 80z" />
      </svg>
    </button>
  </div>

  <div className={`${styles.child} ${styles.child4}`}>
    <button className={`${styles.button} ${styles.btn4}`}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        height="1em"
        viewBox="0 0 320 512"
        fill="#4267B2"
      >
        <path d="M279.14 288l14.22-92.66h-88.91v-60.13c0-25.35 12.42-50.06 52.24-50.06h40.42V6.26S260.43 0 225.36 0c-73.22 0-121.08 44.38-121.08 124.72v70.62H22.89V288h81.39v224h100.17V288z" />
      </svg>
    </button>
  </div>

</div>

          </div>
          
          <div className={styles.linksSection}>
            <div className={styles.linkColumn}>
              <h3>Plateforme</h3>
              <a href="#">Missions</a>
              <a href="#">Associations</a>
            </div>
            
            <div className={styles.linkColumn}>
              <h3>Ressources</h3>
              <a href="#">Guide du bénévole</a>
              <a href="#">Centre d'aide</a>
              <a href="#">Contact</a>
            </div>

          </div>
        </div>
        
        <div className={styles.bottomSection}>
          <p className={styles.copyright}>
            © 2025 VolunteerConnect. Tous droits réservés.
          </p>
          
        </div>
      </div>
    </footer>
  )
}

export default Footer